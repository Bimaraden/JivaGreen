...
export const getEnvironmentalData = async (lat: number, lon: number): Promise<EnvironmentalData> => {
  try {
    ...
    const tempVariation = (Math.random() * 0.8) - 0.4; // +/- 0.4 derajat
    
    const baseHumidity = 65 + (Math.abs(lon) % 15);
    const humVariation = Math.floor(Math.random() * 3) - 1; // +/- 1%

    return {
      aqi: Math.max(0, baseAqi + aqiVariation),
      temp: parseFloat((baseTemp + tempVariation).toFixed(1)),
      humidity: Math.max(0, Math.min(100, baseHumidity + humVariation)),
